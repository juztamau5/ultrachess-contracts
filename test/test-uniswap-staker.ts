...
describe("Uniswap V3 Staker", () => {
  ...
  let incentiveId: string | undefined; // Set when incentive is created
  ...
  const BASE_TOKEN_AMOUNT = ethers.BigNumber.from("1000113100774717342406"); // About 1,000 base tokens
  const USDC_AMOUNT = ethers.utils.parseUnits("1000", 6); // 1,000 USDC
  const CURVE_AAVE_LP_AMOUNT = BASE_TOKEN_AMOUNT; // About 1,000 Curve Aave LP tokens
  ...
  const UNISWAP_LP_AMOUNT = ethers.BigNumber.from("1000113100774717342460"); // About 1,000 units

  const USDC_AMOUNT_2 = USDC_AMOUNT; // 1,000 USDC
  const CURVE_AAVE_LP_AMOUNT_2 = ethers.BigNumber.from(
    "1000112908451449434713"
  ); // About 1,000 Curve Aave LP tokens
  const CURVE_AAVE_GAUGE_SHARES_2 = CURVE_AAVE_LP_AMOUNT_2; // About 1,000 Curve Aave gauge shares

  const USDC_RETURNED = ethers.BigNumber.from("1886271368"); // About 999.471 USDC
  //const BASE_TOKENS_RETURNED = BASE_TOKEN_AMOUNT.sub(1); // About 1,000 base tokens
  //const ASSET_TOKENS_RETURNED = ASSET_TOKEN_AMOUNT.sub(1); // About 1,000 asset tokens
  ...
  it("should approve UniV3 staker spending reward token", async function () {
    this.timeout(60 * 1000);
  ...
  it("should create the UniV3 staker incentive", async function () {
    this.timeout(60 * 1000);
    ...
    const tx: Promise<ethers.ContractTransaction> = uniV3StakerContract
      .connect(deployer)
      .createIncentive(REWARD_AMOUNT);
    ...
    chai
      .expect(tx)
      .to.emit(uniswapV3StakerContract, "IncentiveCreated")
      .withArgs(
        baseTokenContract.address, // rewardToken
        uniswapV3PoolContract.address, // pool
        startTime, // startTime
        endTime, // endTime
        uniV3StakerContract.address, // refundee
    ...
    await chai.expect(tx).to.emit(baseTokenContract, "Approval").withArgs(
      uniV3StakerContract.address, // owner
      uniswapV3StakerContract.address, // spender
      REWARD_AMOUNT // value
    );
    await chai.expect(tx).to.emit(baseTokenContract, "Transfer").withArgs(
  ...
  it("should stake an LP NFT with base token and USDC", async function () {
    ...
    await chai.expect(tx).to.emit(uniV3PoolerContract, "NFTMinted").withArgs(
      ...
      [0, USDC_AMOUNT, 0], // stableAmounts
      BASE_TOKEN_AMOUNT, // baseTokenAmount
      ASSET_TOKEN_AMOUNT, // assetTokenAmount
    ...
    await chai
      ...
      .withArgs(
        ...
        baseIsToken0 ? BASE_TOKEN_AMOUNT : ASSET_TOKEN_AMOUNT, // amount0
        baseIsToken0 ? ASSET_TOKEN_AMOUNT : BASE_TOKEN_AMOUNT // amount1
      );
  ...
  it("should check stake", async function () {
    ...
    const [secondsPerLiquidityInsideInitialX128, liquidity] =
      await uniV3StakerContract.stakes(nftTokenId);

    chai.expect(secondsPerLiquidityInsideInitialX128).to.equal(0);
  ...
  it("should check deposit", async function () {
    ...
    const [owner, numberOfStakes, tickLower, tickUpper] =
      await uniV3StakerContract.deposits(nftTokenId);
  ...
  it("should check rewards", async function () {
    ...
    // TODO
    const rewardsOwed = await uniV3StakerContract.rewards(beneficiaryAddress);
  ...
  it("should check reward info for LP NFT", async function () {
    ...
    chai.expect(secondsInside).to.be.at.least(REWARD_DELAY_HOURS * 60 * 60);
    chai.expect(secondsInside).to.be.at.most(REWARD_DELAY_HOURS * 60 * 60 + 10);
  });
  ...
  it("should stake an LP NFT with only USDC", async function () {
    ...
    await chai
      ...
      .withArgs(
        ...
        uniV3PoolerContract.address, // recipient
        [0, 0, 0], // stableAmounts
        ethers.BigNumber.from("500056454225724717356"), // assetTokenAmount
        ethers.BigNumber.from("331141084417752161817") // baseTokenReturned
      );
    await chai.expect(tx).to.emit(uniV3PoolerContract, "NFTMinted").withArgs(
      uniV3StakerContract.address, // sender
      uniV3StakerContract.address, // recipient
      ...
      [0, USDC_AMOUNT_2, 0], // stableAmounts
      ethers.BigNumber.from("223736431540882138522"), // baseTokenAmount
      ethers.BigNumber.from("500056454225724717356"), // assetTokenAmount
      ethers.BigNumber.from("334485943856315314980") // liquidityAmount
    );
    await chai.expect(tx).to.emit(uniV3StakerContract, "NFTStaked").withArgs(
      beneficiaryAddress, // sender
      beneficiaryAddress, // recipient
    ...
    await chai
      ...
      .withArgs(
        nftTokenId2, // tokenId
        ethers.BigNumber.from("334485943856315314980"), // liquidity
        baseIsToken0
          ? ethers.BigNumber.from("223736431540882138522")
          : ethers.BigNumber.from("500056454225724717356"), // amount0
        baseIsToken0
          ? ethers.BigNumber.from("500056454225724717356")
          : ethers.BigNumber.from("223736431540882138522") // amount1
      );
    ...
    await chai.expect(tx).to.emit(assetTokenContract, "Transfer").withArgs(
      ethers.constants.AddressZero, // from
      uniV3PoolerContract.address, // to
      ethers.BigNumber.from("1000112908451449434713") // value
    );
    await chai.expect(tx).to.emit(assetTokenContract, "Transfer").withArgs(
      uniV3PoolerContract.address, // from
      uniV3SwapperContract.address, // to
      ethers.BigNumber.from("500056454225724717356") // value
    );
    await chai.expect(tx).to.emit(assetTokenContract, "Transfer").withArgs(
    ...
    await chai.expect(tx).to.emit(baseTokenContract, "Transfer").withArgs(
      ...
      uniV3SwapperContract.address, // to
      ethers.BigNumber.from("331141084417752161817") // value
    );
    await chai.expect(tx).to.emit(baseTokenContract, "Transfer").withArgs(
      uniV3SwapperContract.address, // from
      uniV3PoolerContract.address, // to
    ...
    await chai.expect(tx).to.emit(baseTokenContract, "Transfer").withArgs(
      uniV3PoolerContract.address, // from
      uniswapV3PoolContract.address, // to
      ethers.BigNumber.from("223736431540882138522") // value
    );
  });
  ...
  it("should unstake the LP NFT and receive tokens", async function () {
    ...
    await chai.expect(tx).to.emit(uniV3StakerContract, "NFTUnstaked").withArgs(
      ...
      1, // stableIndex
      USDC_RETURNED, // stablesCollected
      USDC_RETURNED, // stablesReturned
      rewardClaimed // baseTokensReturned
    );
    await chai.expect(tx).to.emit(uniV3PoolerContract, "NFTCollected").withArgs(
      ...
      nftTokenId, // nftTokenId
      UNISWAP_LP_AMOUNT, // liquidityAmount
      ethers.BigNumber.from("776376669233835203882"), // baseTokensCollected
      ethers.BigNumber.from("1500169555000442059761"), // assetTokensCollected
      1, // stableIndex (USDC)
      USDC_RETURNED // stablesReturned
    );
    await chai.expect(tx).to.emit(uniV3SwapperContract, "TokensSold").withArgs(
      uniV3PoolerContract.address, // sender
      uniV3PoolerContract.address, // recipient
      1, // stableIndex (USDC)
      ethers.BigNumber.from("776376669233835203882"), // baseTokenSpent
      ethers.BigNumber.from("387064557"), // stablesUnstaked
      0 // stablesReturned
    );
    /* TODO
    await chai
      .expect(tx)
      .to.emit(curveAaveStakerContract, "GaugeUnstakedOneToken")
      .withArgs(
        uniV3PoolerContract.address, // sender
        uniV3PoolerContract.address, // recipient
        CURVE_AAVE_GAUGE_SHARES.sub(1), // gaugeShares
        1, // stableIndex (USDC)
        USDC_RETURNED, // stablesUnpooled
        USDC_RETURNED, // stablesUnstaked
        USDC_RETURNED // stablesReturned
      );
    await chai
      .expect(tx)
      .to.emit(curveAavePoolerContract, "LiquidityRemovedOneToken")
      .withArgs(
        curveAaveStakerContract.address, // sender
        curveAaveStakerContract.address, // recipient
        CURVE_AAVE_LP_AMOUNT.sub(1), // lpTokenAmount
        1, // stableIndex (USDC)
        USDC_RETURNED, // stablesRemoved
        USDC_RETURNED // stablesReturned
      );
    ...
    await chai
      ...
      .withArgs(
        nftTokenId, // tokenId
        UNISWAP_LP_AMOUNT, // liquidity
        baseIsToken0 ? BASE_TOKENS_RETURNED : ASSET_TOKENS_RETURNED, // amount0
        baseIsToken0 ? ASSET_TOKENS_RETURNED : BASE_TOKENS_RETURNED // amount1
      );
    await chai
      .expect(tx)
